...
<!-- SIGNATURE: sha256:8dbc2a731081ebf8f7640eada24b72d0b3c4692ad8c7a5f2119978c77b55bafe -->
...
**Workspace scope.** Read-only and local-execution commands such as `status`, `check`, `list`, `show`, `search`, `verify`, `describe`, and loop execution. Workspace-scoped commands MUST run in any workspace and MUST NOT mutate governed artifacts.
...
A claim MUST record the owning workspace and a last-activity timestamp. Any govctl invocation in the claiming workspace SHOULD refresh that timestamp for its held claims. A claim whose owning workspace no longer exists, or whose last activity exceeds a configurable inactivity period, MUST be treated as expired and MUST NOT block operations.
...
**Rationale:** Version-semantics operations on one RFC cannot merge meaningfully when performed on two branches at once — both would edit the same version and changelog fields — so they are serialized by an exclusive claim. Content edits to independent clause files merge cleanly, so they receive visibility rather than exclusion.
