...
<!-- SIGNATURE: sha256:1028b7bb34572f1d165341a1c930d3a7019189435f58a1fb73a524b87e4d0f8a -->
...
Lifecycle readiness depends on companion amendments integrating the new resource into the closed contracts owned by [RFC-0000:C-REFERENCE-HIERARCHY](../rfc/RFC-0000.md#rfc-0000c-reference-hierarchy), [RFC-0002:C-RESOURCES](../rfc/RFC-0002.md#rfc-0002c-resources), [RFC-0002:C-CRUD-VERBS](../rfc/RFC-0002.md#rfc-0002c-crud-verbs), [RFC-0002:C-EDIT-FIELD-CONTRACT](../rfc/RFC-0002.md#rfc-0002c-edit-field-contract), [RFC-0002:C-SHOW-PROJECTION](../rfc/RFC-0002.md#rfc-0002c-show-projection), [RFC-0002:C-GLOBAL-COMMANDS](../rfc/RFC-0002.md#rfc-0002c-global-commands), [RFC-0002:C-SEARCH-COMMAND](../rfc/RFC-0002.md#rfc-0002c-search-command), [RFC-0002:C-COMPATIBILITY-BOUNDARY](../rfc/RFC-0002.md#rfc-0002c-compatibility-boundary), and [RFC-0002:C-OUTPUT-FORMAT](../rfc/RFC-0002.md#rfc-0002c-output-format). Until those amendments remove conflicts with existing exhaustive lists, this draft remains a proposal rather than a finalizable contract.
...
A Conformance Case has no persisted lifecycle status. Its requirement bindings derive `current`, `provisional`, or `stale` applicability under [RFC-0008:C-TRACE-QUERY](../rfc/RFC-0008.md#rfc-0008c-trace-query). Editing or deleting a Case changes only the current declaration; repository version control remains the history mechanism. Case mutation MUST NOT modify RFC versions, changelogs, Clause metadata, or sealed content signatures.
...
3. Verification Guards and Work Items MUST NOT persist reverse Conformance Case lists.
...
RFC `refs`, ADR `refs`, and Work Item `refs` MUST NOT identify a Conformance Case. `[[...]]` link targets and known Conformance Case ID mentions in governed RFC, ADR, or Work Item prose MUST NOT identify a Conformance Case. Work Item completion policy MUST continue to select Verification Guards through the existing required-Guard surface rather than through Cases.
...
- a governed RFC, ADR, or Work Item reference surface identifies a Conformance Case.
...
The resource MUST support `list`, `get`, `show`, `edit`, and `delete` with the applicable shared behavior defined by [RFC-0002:C-CRUD-VERBS](../rfc/RFC-0002.md#rfc-0002c-crud-verbs) and [RFC-0002:C-SHOW-PROJECTION](../rfc/RFC-0002.md#rfc-0002c-show-projection). `get <id> [field]` MUST expose `id`, `title`, `tags`, `path`, `selector`, `requirements`, and `guards`. Because Cases have no lifecycle state, current and archive `show` projections MUST be content-equivalent.
...
Wire-layout prefixes such as `govctl.` and `case.` MUST be rejected. Every failed creation, edit, or deletion MUST leave governed files unchanged.
...
Global search MUST index Conformance Case IDs, titles, tags, paths, selectors, requirement refs and versions, and Guard IDs. `govctl search --type conformance` MUST select this resource type.
...
- `current` when its owning RFC is normative, the Clause is active, the Clause has a `since` version, and the binding version equals the RFC current version; or
- `stale` otherwise.
...
The command MUST validate the complete Conformance Case graph before emitting data. Structural errors MUST produce a non-zero exit status and no partial result. Stale or provisional bindings and Cases without Guards MUST remain successful trace results.
