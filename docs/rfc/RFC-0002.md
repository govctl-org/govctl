...
<!-- SIGNATURE: sha256:a22a949ee405c904e6a099680216d5ce477229dd950e31eb7168821cf1d53da5 -->
...
- The `govctl.schema` field in TOML artifacts MUST select the corresponding schema version for that artifact type
...
Performs deterministic repository-local format migration for legacy governance artifacts.

Syntax: `govctl migrate [--dry-run]`

Behavior:
- Detects legacy JSON RFC and clause files in the current repository
- Converts legacy JSON RFC and clause files to TOML equivalents
- Updates govctl-managed internal RFC clause-path references from `.json` paths to `.toml` paths
- Applies any deterministic file-structure upgrades required by current TOML artifact schemas, including release-file metadata upgrades
- Preserves artifact IDs, ordering, and semantic content during conversion
- MUST validate converted artifacts before reporting success
- MUST leave the repository unchanged if conversion or validation fails at any step
- MUST be safe to run on an already-migrated repository and report a no-op result in that case
