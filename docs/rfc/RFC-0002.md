...
<!-- SIGNATURE: sha256:91f780b62531eb6c1b6cb779181ee87fe032f527f1fe3a1ace1d582d6c403f8f -->

# RFC-0002: CLI Resource Model and Command Architecture

> **Version:** 2.0.0 | **Status:** normative | **Phase:** stable
> **Owners:** @govctl-org
...
`new` MUST be supported for RFCs, ADRs, Work Items, Clauses, and Guards. It MUST validate inputs. It MUST generate or validate the resource ID. It MUST initialize schema-defined defaults and create the canonical storage structure.
...
`edit` MUST be supported for RFCs, ADRs, Work Items, Clauses, and Guards. It MUST be the only universal field-mutation subcommand for those resources. These resources MUST NOT expose `set`, `add`, `remove`, or `tick` as sibling mutation subcommands.
...
`delete` MUST be supported for Work Items, Clauses, and Guards. RFCs and ADRs MUST use their lifecycle operations instead.

For Guards, deletion MUST reject references from Work Item verification requirements, Work Item waivers, and project default Guards.
...
- `govctl {rfc|clause|adr|work|guard} edit <ID> tags --add <tag>` - assign a tag to an artifact
- `govctl {rfc|clause|adr|work|guard} edit <ID> tags --remove <tag>` - remove a tag from an artifact
...
RFC, ADR, Work Item, Clause, and Guard resource commands MUST expose `show <id>` as a side-effect-free projection operation distinct from `get`. This distinction is a scoped exception to [RFC-0002:C-CRUD-VERBS](../rfc/RFC-0002.md#rfc-0002c-crud-verbs): `get` retrieves a field or complete structured resource, while `show` presents either the current or archival projection defined here.
...
For resources without a `deprecated` or `superseded` lifecycle state, including Work Items and Guards, `current` and `archive` MUST be content-equivalent. Other lifecycle states MUST remain fully rendered.

The `archive` projection MUST render all authoritative content supported by the resource's human-readable renderer without lifecycle-based omission. In particular, it MUST include every RFC section, Clause text, RFC changelog entry, ADR content section and alternative, Work Item content section, and Guard field present in the selected resource. Terminal styling and transport-only wrappers MAY differ. `render` MUST always use the `archive` projection and MUST NOT suppress deprecated or superseded content.
...
- RFC, Clause, ADR, Work Item, and Guard TOML files MUST NOT contain the legacy `[govctl].schema` field.
