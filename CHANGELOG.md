...
Repositories that drive agents in separate git worktrees or jj workspaces of one
clone now share a coordination registry living in VCS-shared storage, while
governance content stays versioned with the branch. Single-checkout and
non-version-controlled projects behave exactly as before.

### Added

- Every command is classified as workspace, branch-content, or trunk scoped.
  `govctl release`, `govctl release undo`, and `govctl migrate` are
  trunk-scoped and refuse to run outside the primary workspace, with a
  diagnostic naming it. (WI-2026-09-08-001)
- The primary workspace is detected from git metadata automatically; jj
  projects treat the workspace named `default` as primary and can override it
  with `[workspace] primary = "<name>"` in `gov/config.toml`.
  (WI-2026-09-08-001, WI-2026-09-08-006)
- Artifact creation reserves generated IDs in a per-clone registry, so two
  workspaces can never assign the same RFC, ADR, or work item ID. Allocation
  consults shared version-control history, so IDs of merged or deleted
  artifacts are never reused. (WI-2026-09-08-002)
- `rfc bump`, `finalize`, `advance`, `deprecate`, and `supersede` acquire an
  exclusive claim on every RFC they mutate and fail without mutation when
  another workspace holds a live claim; content edits warn without blocking.
  Claims expire after a configurable inactivity period and can be released or
  taken over with the new `govctl claim list/release/steal` commands, with
  takeovers recorded in an audit trail. (WI-2026-09-08-003)
- `govctl status` lists work items actively claimed by other workspaces in a
  new "Active in Other Workspaces" section; output is unchanged when no other
  workspace is active. (WI-2026-09-08-004)

### Fixed

- Read-only commands never block on the registry lock, and detected registry
  corruption or write failure degrades to single-checkout behavior with a
  warning instead of failing the command. (WI-2026-09-08-005,
  WI-2026-09-08-007)
- The registry works in jj secondary workspaces by following the `.jj/repo`
  pointer file. (WI-2026-09-08-005)
- Reservations are pruned only once the artifact is witnessed in shared
  version-control history, closing an ID-reuse window for unmerged deletions.
  (WI-2026-09-08-005)
- Bare-repo and nested VCS layouts resolve workspace detection toward a
  warning instead of silently misidentifying the primary workspace.
  (WI-2026-09-08-005)
- Registry edge cases: claim steal records its audit event only after the
  transfer succeeds, corrupt reservations are quarantined aside once instead
  of degrading every later allocation, and a failed supersede releases the
  claims it already acquired. (WI-2026-09-08-008)
...
Codex and Claude can now install the same governed workflows through their
native plugin systems while retaining client-specific agent definitions and
hook protocols.

### Added

- `govctl agent doctor|install|update <codex|claude|all>` checks runtime
  readiness and manages the bundled integration without requiring an initialized
  govctl project.
- Native plugin bundles install shared skills and hooks for Codex and Claude,
  alongside Claude Markdown agents and standalone Codex TOML reviewer roles.
- Session-start hooks provide bounded active-work and loop context inside
  governed projects, remain silent elsewhere, and offer recovery guidance for
  invalid governance state.

### Changed

- Codex reviewer roles follow the current custom-agent contract and apply
  read-only execution constraints to review-only roles.
- Workflow skills name exact reviewer agent types where deterministic delegation
  matters.
- Claude and Codex use runtime-specific hook manifests and protocol adapters
  while sharing the same governance semantics.
- Direct lifecycle-artifact edits receive non-blocking guidance toward canonical
  govctl commands. Hooks do not repeat project-wide validation after edits or at
  session stop.

### Fixed

- Codex-format installation resolves the format-implied `.codex` directory when
  no explicit destination is configured.
- Session-start context retains usable project state alongside non-fatal Work
  Item warnings.
- Hook path handling now supports relative project discovery, filesystem roots,
  CRLF patch headers, and trailing whitespace.
- Shell completions expose public agent-management commands without leaking the
  internal hook adapter.
