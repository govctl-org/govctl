...
fn main() {
    ...
    println!("cargo:rerun-if-changed=.claude-plugin/marketplace.json");
    println!("cargo:rerun-if-changed=.claude/.claude-plugin");
    println!("cargo:rerun-if-changed=.claude/.codex-plugin");
...
fn generate_plugin_assets() -> Result<(), Box<dyn Error>> {
    let plugin_root = Path::new(".claude");
    let mut files = Vec::new();
    for relative_root in [
        ...
        collect_files(&plugin_root.join(relative_root), &mut files)?;
    }
    files.sort();

    let mut out = String::new();
    out.push_str("// @generated by build.rs from the native agent plugin bundle\n");
    out.push_str("// Do not edit manually.\n\n");
    out.push_str("pub const PLUGIN_ASSETS: &[(&str, &str)] = &[\n");
    for file in files {
        let relative = file
            .strip_prefix(plugin_root)
            .map_err(|e| format!("failed to relativize plugin asset {}: {e}", file.display()))?;
        let relative = path_to_slash_string(relative)?;
        let source = format!("/.claude/{relative}");
        out.push_str(&format!(
            "    ({:?}, include_str!(concat!(env!(\"CARGO_MANIFEST_DIR\"), {:?}))),\n",
            relative, source
        ));
    ...
    out.push_str("];\n");
    out.push_str(
        "pub const PLUGIN_MARKETPLACE_TEMPLATE: &str = include_str!(concat!(env!(\"CARGO_MANIFEST_DIR\"), \"/.claude-plugin/marketplace.json\"));\n",
    );

    let out_dir = std::env::var("OUT_DIR")?;
    let out_path = Path::new(&out_dir).join("agent_plugin_assets.rs");
    fs::write(out_path, out)?;
    Ok(())
}
